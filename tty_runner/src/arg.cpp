...
enum class Option { HELP, PATH_TO_MAKEFILE, COMMAND_LIST, OUTPUT_FILE };
...
Arg::Arg(int argc, char *argv[]) {
  ...
  if (vm.count(option_names.at(Option::PATH_TO_MAKEFILE))) {
    this->path_to_makefile =
        vm[option_names.at(Option::PATH_TO_MAKEFILE)].as<string>();
  } else {
    throw runtime_error("path to makefile is required");
  }

  if (vm.count(option_names.at(Option::COMMAND_LIST))) {
    this->command_list_file =
        vm[option_names.at(Option::COMMAND_LIST)].as<string>();
  } else {
